...
let data_gt = [
    ...
    },
    {
        role: "user",
        content:
            "여행계획은 이 양식으로 전달될거야. '${target} 여행을 떠날거야. 여행 시작일은 ${start_date[0]}이고, 여행지에 도착하는 시간은 ${start_date[1]}야. 여행이 끝나는 날은 ${end_date[0]}고, ${end_date[1]}까지 공항이나 역에 도착해야해. 그 외의 여행 조건은 다음과 같아.${main_target}' 그 외의 여행 조건은 없을수도 있어. 없으면 자유여행을 하면 돼.",
    },
    {
        role: "user",
        content: `하루의 일정은 출발일의 도착시간과 종료일의 출발시간을 고려해서 아침식사, 오전, 점심식사, 오후, 저녁식사, 야간 일정이 가능한 한 들어가야하고 식사일정을 짤때는 추천 메뉴를 적어줘. 세부일정도 모두 네가 짜줘야 하는 부분이야. 거리와 시간을 고려해서 일정을 짜줘. 일정 내부에서는 ' 를 사용해선 안돼. 추가적인 질문은 받지 않아. 전해준 데이터만으로 여행계획을 세워줘.  
        `,
    },
    {
        role: "user",
    ...
    {
        role: "user",
        content: `여행계획은 다음과 같은 JSON 형식으로 전달해줘야해. 형식 = {"$일차" : {"날짜":"$일차($년 $월 $일, $요일)", "일정":"["시간:분  = 세부일정", "시간:분 = 세부일정", ...]"}, "$일차" : {"날짜":"$일차($년 $월 $일, $요일)", "일정":"["시간:분  = 세부일정", "시간:분  = 세부일정", ...]"} }. 이 양식은 절대로 지켜져야해.
...
const createQuestion_gt = (_) => {
    ...
    if (main_target == undefined) {
        return `${target} 여행을 떠날거야. 여행 시작일은 ${start_date[0]}이고, 여행지에 도착하는 시간은 ${start_date[1]}야. 여행이 끝나는 날은 ${end_date[0]}고, ${end_date[1]}까지 공항이나 역에 도착해야해.`;
    }
    return `${target} 여행을 떠날거야. 여행 시작일은 ${start_date[0]}이고, 여행지에 도착하는 시간은 ${start_date[1]}야. 여행이 끝나는 날은 ${end_date[0]}고, ${end_date[1]}까지 공항이나 역에 도착해야해. 그 외의 여행 조건은 다음과 같아.${main_target}`;
};
